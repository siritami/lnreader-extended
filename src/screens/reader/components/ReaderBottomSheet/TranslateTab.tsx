      ...
      >
        ...
        <ScrollView style={styles.languageList}>
          {supportedLanguagesList.map(lang => (
            ...
            >
              <Text style={[styles.languageItemText, { color: theme.onSurface }]}>
                {lang.label}
              </Text>
              {currentLang === lang.value && (
    ...
    <>
      ...
      >
        <View style={styles.section}>
          <List.SubHeader theme={theme}>{getString('readerScreen.bottomSheet.translateTab.translationSettings')}</List.SubHeader>

          <View style={styles.row}>
             <Text style={[styles.label, { color: theme.onSurface }]}>{getString('readerScreen.bottomSheet.translateTab.engine')}</Text>
             <View style={styles.buttonGroup}>
               <Button
                 title={getString('readerScreen.bottomSheet.translateTab.googleFree')}
               ...
               <Button
                 title={getString('readerScreen.bottomSheet.translateTab.llmAPI')}
          ...
          >
            <Text style={[styles.label, { color: theme.onSurface }]}>
              {getString('readerScreen.bottomSheet.translateTab.sourceLanguage')}
            </Text>
          ...
          >
            <Text style={[styles.label, { color: theme.onSurface }]}>
              {getString('readerScreen.bottomSheet.translateTab.targetLanguage')}
            </Text>
            <Text style={[styles.value, { color: theme.onSurfaceVariant }]}>
          ...
          {engine === 'llm' && (
            <View style={styles.llmConfigSection}>
              <List.SubHeader theme={theme}>{getString('readerScreen.bottomSheet.translateTab.llmAPI')} Configuration</List.SubHeader>
              ...
              <Menu
                ...
                anchor={
                  ...
                  >
                    <Text style={{ color: theme.onSurfaceVariant }}>{getString('readerScreen.bottomSheet.translateTab.provider')}</Text>
                    <Text style={{ color: theme.onSurface }}>{getProviderLabel(llmProvider)}</Text>
                  </TouchableOpacity>
                }>
                {PROVIDERS.map(p => (
                  <Menu.Item
              ...
              <View style={styles.modelRow}>
                <TextInput
                  label={getString('readerScreen.bottomSheet.translateTab.modelName')}
                  value={llmModel}
                  onChangeText={text => setTranslateSettings({ llmModel: text })}
                  mode="outlined"
                  style={[styles.input, { flex: 1, marginBottom: 0 }]}
                  theme={{ colors: { primary: theme.primary, background: theme.surface, onSurface: theme.onSurface, onSurfaceVariant: theme.onSurfaceVariant } }}
                />
                <Button 
                   title={getString('readerScreen.bottomSheet.translateTab.loadModels')} 
                   mode="contained" 
                   onPress={loadModels}
                   style={{ marginLeft: 8 }}
              ...
              <TextInput
                label={getString('readerScreen.bottomSheet.translateTab.systemPrompt')}
                value={llmSystemPrompt}
                onChangeText={text => setTranslateSettings({ llmSystemPrompt: text })}
                mode="outlined"
                multiline
                numberOfLines={4}
      ...
      <Portal>
        ...
        >
          <Text style={[styles.modalTitle, { color: theme.onSurface }]}>Select Model</Text>
          <ScrollView style={styles.languageList}>
             {availableModels.map(m => (
                <TouchableOpacity
                   key={m}
                   style={[styles.languageItem, llmModel === m && { backgroundColor: theme.surfaceVariant }]}
                   onPress={() => {
                      setTranslateSettings({ llmModel: m });
                      setModelPickerVisible(false);
                   }}
                >
                   <Text style={[styles.languageItemText, { color: theme.onSurface }]}>{m}</Text>
                   {llmModel === m && <Text style={[styles.checkIcon, { color: theme.primary }]}>✓</Text>}
                </TouchableOpacity>
             ))}
          </ScrollView>
          <Button title={getString('common.cancel')} mode="outlined" onPress={() => setModelPickerVisible(false)} style={styles.cancelButton} />
        </Modal>
      </Portal>
    </>
  );
